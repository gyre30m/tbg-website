-- Supabase-Compatible Database Schema for Hierarchical Authentication System
...
CREATE POLICY "Firm members can view their own firm" ON public.firms
    FOR SELECT USING (
        id IN (
...
CREATE POLICY "Firm admins can manage their firm invitations" ON public.user_invitations
    FOR ALL USING (
        firm_id IN (
            SELECT firm_id FROM public.user_profiles 
            WHERE user_id = auth.uid() AND role = 'firm_admin'

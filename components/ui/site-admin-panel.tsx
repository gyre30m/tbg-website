...
export function SiteAdminPanel() {
  ...
  const createFirmClient = async (
    firmData: Omit<CreateFirmFormData, "adminEmail">,
    imageUrl?: string
  ): Promise<Firm | null> => {
    try {
      console.log(
        "createFirmClient called with:",
        firmData,
        "imageUrl:",
        imageUrl
      );
  ...
  const inviteUserToFirmClient = async (
    ...
    try {
      ...
      if (!firm || firm.domain !== emailDomain) {
        return {
          success: false,
          error: "Email domain does not match firm domain",
        };
      }
  ...
  const fetchAllForms = async () => {
    try {
      ...
      fetchPromises.push(
        Promise.resolve(wdQuery.order("updated_at", { ascending: false }))
          .then(({ data, error }) => {
      ...
      fetchPromises.push(
        Promise.resolve(wtQuery.order("updated_at", { ascending: false }))
          .then(({ data, error }) => {
            if (error && error.code !== "PGRST116") throw error; // Ignore "not found" errors
            return (data || []).map((form) => ({
              ...form,
  ...
  const fetchSiteAdmins = async () => {
    try {
      ...
      // Get current user to check if we can access user data
      ...
      // Get user_profiles with site_admin role
      ...
      if (profilesError) {
        console.error("Error fetching site admin profiles:", profilesError);
  ...
  const handleCreateFirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const firm = await createFirmClient(
        firmData,
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Manage law firms and view all forms
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Badge variant="outline" className="flex items-center gap-1">
                  ...
                  {siteAdmins.length} Site Admin
                  {siteAdmins.length !== 1 ? "s" : ""}
                </Badge>
              </div>
            </div>
